...
| 分析文件内容提取用户信息 | 检查任务状态 |
| 生成 .info/usr.json | 调用其他 skills |
| 检测文件变化判断是否需要更新 | 管理任务生命周期 |
| 响应用户的身份查询（"我是谁"） | 决定何时生成技能 |
...
- **对 commander**：被动提供数据，不关心对方如何使用
- **对 skill-generator**：只提供画像数据，不参与技能分析
- **独立性**：可独立运行，不依赖其他 skills
